...
import { Text, View, Alert, StyleSheet, ImageBackground, SafeAreaView, TouchableOpacity, Image, StatusBar, Platform, Linking, ScrollView } from 'react-native';
...
export default class DailyPicScreen extends React.Component{
  ...
  getAPOD = () =>{
    axios
    .get('https://api.nasa.gov/planetary/apod?api_key=4YqyaVqvqAnCmuaL2ED92TYvJyMkivl6EwUgTyui')
    .then(response => {
      this.setState({
        apod: response.data
      })
  ...
  renderVideo = () => { 
    <TouchableOpacity 
      ...
      onPress={() => Linking.openURL(this.state.apod.url).catch(err => console.error("Couldn't load page", err))}>     <View style={styles.iconContainer}> 
      ...
      return (
        ...
        <ImageBackground
          ...
          <ScrollView style = {styles.listContainer}>
            <TouchableOpacity 
              style = {styles.container} /*styles.container for now until I get the correct styles*/
